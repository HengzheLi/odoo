    ...
    if tools.config['syslog']:
        ...
        try:
            ...
            if tools.config['logrotate'] is not False:
                ...
                handler = logging.handlers.TimedRotatingFileHandler(filename=logf, when='D', interval=1, backupCount=30, encoding='utf-8')
            elif os.name == 'posix':
                handler = logging.handlers.WatchedFileHandler(logf)
            else:
                handler = logging.FileHandler(logf, 'w', 'utf-8')
        except Exception:
